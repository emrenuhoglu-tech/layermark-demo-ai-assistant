...
4. **`02-memory/training/MASTER-PROMPT-UNIFIED.md`** (or fallback `C:\Users\emren\.layermark\pylib\training\MASTER-PROMPT-UNIFIED.md`) — the 20 principles, 4 phases, output template.
5. **`02-memory/training/00-INDEX.md`** (or fallback `C:\Users\emren\.layermark\pylib\training\00-INDEX.md`) — index of 19 modules. Grep for the relevant module names by topic.
...
3. **Surface implicit constraints.** The user rarely restates global rules (Surgical Changes, no hardcoded selectors, TR IP for A2-A9, no .env commits). Pull them in if they apply.
...
User: "casino bot'ta A1 login script'inin loglarini debug yap"
...
Execute on:
- **Who:** main Claude session in casino bot project
...
User: "A2 login test ekleyelim"

Doctrine pulled:
- `CLAUDE.md:Risk & limit` — A2-A9 must run on TR residential IP; A1 is sacrificial
...
Structured prompt:
```
Add an A2 login smoke test mirroring the existing A1 path in scripts/login_once.py.

Constraints (project doctrine):
- A2 traffic must exit via the proxy in .env A2_PROXY (TR residential). Fail loudly if A2_PROXY is empty.
- All selectors must come from config/sites/pokerklas752.yaml — no hardcoded selectors in the script.
- Persist session state to data/sessions/A2/ following the same shape as A1.
- Surgical Changes: don't refactor login_once.py for A1. If you must share logic, extract a single helper without altering A1's behavior.

Success: `python -m scripts.login_once --account A2` lands in the live-casino lobby (selector defined in the YAML) and writes storage_state.json. Test once with HEADLESS=false to verify visually before committing.
```
...
Structured prompt:
```
You are running as a weekly cron job in the layermark-casino-bot repo. The repo is already cloned.
...
- **How to invoke:** create with RemoteTrigger action:create, cron `0 7 * * 1` (Monday 10:00 TR), source = layermark-casino-bot repo
